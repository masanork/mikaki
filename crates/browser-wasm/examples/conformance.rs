...
#[derive(Deserialize)]
struct Request {
    ceremony: mikaki_auth::Ceremony,
    browser_hash: String,
    now: u64,
    max_failures: u32,
    response: serde_json::Value,
    credential: Option<mikaki_webauthn::StoredCredential>,
}
fn verify(input: &str) -> Option<serde_json::Value> {
    mikaki_webauthn::strict_json(input, 8_388_608, 16).ok()?;
    let raw: serde_json::Value = serde_json::from_str(input).ok()?;
    if let Some(mds) = raw.get("mds") {
        let verified =
            mikaki_webauthn::metadata::verify_mds(serde_json::from_value(mds.clone()).ok()?)
                .ok()?;
        return serde_json::to_value(verified).ok();
    }
    mikaki_webauthn::strict_json(input, 262144, 16).ok()?;
