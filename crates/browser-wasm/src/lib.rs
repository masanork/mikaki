...
pub fn valid_json(input: &str, bytes: usize, depth: usize) -> bool {
    mikaki_webauthn::strict_json(input, bytes, depth).is_ok()
}
...
pub fn diagnostic_stage(code: &str) -> Option<String> {
    mikaki_webauthn::Invalid::from_code(code).map(|e| e.stage().to_owned())
}
...
fn diagnostic(error: mikaki_webauthn::Invalid) -> JsValue {
...
fn parse<T: serde::de::DeserializeOwned>(input: &str) -> Result<T, JsValue> {
    ...
    mikaki_webauthn::bounded_json(input, 262144, 16).map_err(diagnostic)?;
    serde_json::from_str(input).map_err(|_| diagnostic(mikaki_webauthn::Invalid::Input))
}
#[wasm_bindgen]
pub fn register(input: &str) -> Result<String, JsValue> {
    let i: Input<mikaki_webauthn::Registration> = parse(input)?;
    let proof = i
        .ceremony
...
pub fn authenticate(input: &str, credential: &str) -> Result<String, JsValue> {
    let i: Input<mikaki_webauthn::Assertion> = parse(input)?;
...
pub fn authorize(
    ...
    let Ok(request) = serde_json::from_str::<mikaki_oidc::Authorization>(input) else {
        return false;
    };
    request
...
pub fn pkce(verifier: &str) -> Result<String, JsValue> {
    mikaki_oidc::pkce(verifier).ok_or_else(|| JsValue::from_str("invalid_grant"))
}

#[wasm_bindgen]
pub fn attestation_hint(input: &str) -> Result<String, JsValue> {
    mikaki_webauthn::attestation_hint(input).map_err(diagnostic)
}
#[wasm_bindgen]
pub fn mds_crl_urls(jwt: &str) -> Result<String, JsValue> {
    let urls = mikaki_webauthn::metadata::mds_crl_urls(jwt).map_err(diagnostic)?;
...
pub fn verify_mds(input: &str) -> Result<String, JsValue> {
    ...
    let verified = mikaki_webauthn::metadata::verify_mds(i).map_err(diagnostic)?;
