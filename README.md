...
RPからmikakiへ接続する際の手順は[RP向け接続手順](docs/rp-integration.md)を参照。

本書は開発方針を記録する。現在の実装・配備状態は[Cloudflare deployment](docs/cloudflare-deployment.md)を参照。安全性の認証を示すものではない。

**開発中の試作です。本番ドメインに準備配備していますが、RP登録・ログアウト連携・公開前検証は完了していません。利用者向けサービスとして運用しないでください。**
