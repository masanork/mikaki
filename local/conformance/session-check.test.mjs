...
import { activateWorkerPolicy } from '../../scripts/worker-policy-store.mjs';
...
test('session check authenticates its RP, binds sid, and observes D1 revocation and lease changes', async () => {
  ...
  try {
    ...
    const runtime = JSON.parse(
      await readFile(new URL('../generated/worker-policy.json', import.meta.url)),
    );
    ...
    const shorter = await (await check('owned-sid')).json();
