...
import { initSync, register } from '../../crates/browser-wasm/pkg/sakimori_browser_wasm.js';
import { webauthnDiagnostic } from '../webauthn-errors.mjs';
...
test('Wasm rejection carries only a reason code and stage across the JS boundary', async () => {
  ...
  const rejects = (value, code, stage) =>
    assert.throws(
      () => register(value),
      (error) => {
        assert.deepEqual(JSON.parse(error), { code, stage });
...
test('Wasm registration exposes trusted attestation evidence only after verification', async () => {
  ...
  assert.throws(
    ...
    (error) => {
      assert.deepEqual(JSON.parse(error), { code: 'trust', stage: 'attestation' });
      return true;
    },
  );
  ...
  assert.throws(
    () => register(JSON.stringify(input)),
    (error) => {
      assert.deepEqual(JSON.parse(error), { code: 'input', stage: 'input' });
      return true;
    },
  );
});
