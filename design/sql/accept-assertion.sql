...
SELECT :client_id,:jti,:endpoint,:assertion_operation_id,:retain_until
...
VALUES(:assertion_operation_id, CASE WHEN EXISTS (
  SELECT 1 FROM assertion_use WHERE accepted_by=:assertion_operation_id
    AND client_id=:client_id AND jti=:jti AND endpoint=:endpoint
) THEN 1 ELSE 0 END);
DELETE FROM atomic_guard WHERE operation_id=:assertion_operation_id;
