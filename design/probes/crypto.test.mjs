...
const fixtureKey = await importJWK({ ...publicJwk,
  d: base64url.encode(Buffer.from('c9afa9d845ba75166b5c215767b1d6934e50c3db36e89b127b8a622b120f6721', 'hex')),
}, 'ES256');
const now = 1893456000;
const claims = { iss: 'https://issuer.invalid', aud: 'fixture-client', sub: 'fixture-sub', iat: now, exp: now+300 };
const options = { algorithms: ['ES256'], issuer: claims.iss, audience: claims.aud, currentDate: new Date(now*1000), clockTolerance: 0 };
...
test('changed payload is rejected by all verifiers', async () => {
  ...
  await assert.rejects(jwtVerify(`${changed}.${signature}`, publicKey, options), { code: 'ERR_JWS_SIGNATURE_VERIFICATION_FAILED' });
});
...
test('wrong algorithm is rejected rather than chosen from an untrusted header', async () => {
  ...
  await assert.rejects(jwtVerify(`${changed}.${wasm.fixture_sign(changed)}`, publicKey, options), { code: 'ERR_JOSE_ALG_NOT_ALLOWED' });
});

test('correct signature does not override expiry, issuer, or audience', async () => {
  for (const modified of [
    { ...claims, exp: now }, { ...claims, iss: 'https://other.invalid' }, { ...claims, aud: 'other-client' },
  ]) {
