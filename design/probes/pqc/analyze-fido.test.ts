...
function transcript(algorithm) {
  ...
  const key =
    algorithm === -49
      ? new Map<number, number | Uint8Array>([
          [1, 7],
          [3, -49],
          [-1, (signing as ReturnType<typeof ml_dsa65.keygen>).publicKey],
        ])
      : (() => {
  ...
  const signature =
    algorithm === -49
      ? ml_dsa65.sign(signed, (signing as ReturnType<typeof ml_dsa65.keygen>).secretKey)
      : sign('sha256', signed, (signing as { privateKey: KeyObject }).privateKey);
  return {
    ...
    registration: {
      ...
      attestationObject: b64(
        encode(
          new Map<string, string | Buffer | Map<never, never>>([
            ['fmt', 'none'],
