...
export function analyze(transcript) {
  ...
  const valid =
    algorithm === -49
      ? ml_dsa65.verify(signature, signed, verifier)
      : verify('sha256', signed, verifier, signature);
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (!path) throw new Error('usage: node design/probes/pqc/analyze-fido.mjs result.json');
