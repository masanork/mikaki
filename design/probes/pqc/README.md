...
An offline native-only [`recipient_key`](src/bin/recipient_key.rs) CLI prepares an ML-KEM-768 UserInfo recipient seed and public record for the later key lifecycle. It creates the private seed file with owner-only permissions outside the repository, never prints the seed, and can verify its matching public record. Do not create a production key until the dedicated claim Worker and Secrets Store binding are ready. The CLI does not activate a D1 key.
