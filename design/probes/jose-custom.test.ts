...
test('async WebCrypto signature can be supplied as jsonwebtoken Jws in Native and Wasm', async () => {
  ...
  const protectedPart = base64url.encode(
    JSON.stringify({ alg: 'ES256', kid: pair.jwk.kid, typ: 'JWT' }),
  );
  ...
  const signature = new Uint8Array(
    await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, pair.privateKey, signingInput),
  );
  const jws = {
  ...
  assert.equal(
    nativeJws({ ...jws, payload: base64url.encode('{"sub":"tampered"}') }, pair.jwk),
    false,
  );
});
...
test('custom provider applies issuer, audience, expiry, and duplicate-claim checks', async () => {
  for (const algorithm of ['ES256', 'RS256']) {
    const pair =
      algorithm === 'ES256'
        ...
        : await (async () => {
            ...
            return {
              privateKey,
              jwk: { ...(await exportJWK(publicKey)), alg: 'RS256', kid: `claims-${algorithm}` },
            };
          })();
    ...
    for (const invalidCase of invalidCases) {
      const invalidToken = await tokenFor(
        algorithm,
        pair.privateKey,
        pair.jwk.kid,
        'invalid',
