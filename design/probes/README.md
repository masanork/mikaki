...
`token-exchange.mjs` additionally uses `local/generated/worker-policy.json` (generate it with `npm run build:policy`), applies the isolated local D1 migration, seeds synthetic client/session rows, and tests `/authorize` through private_key_jwt token exchange, UserInfo, replay revocation, and a concurrent code exchange with exactly one winner. Its signing keys, browser session cookie, and identifiers are generated for the test run. It does not connect to a remote database or Cloudflare account.
