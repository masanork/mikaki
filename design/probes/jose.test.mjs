...
async function makeToken(algorithm, privateKey, kid, {
  sub = 'probe-subject', tokenIssuer = issuer, tokenAudience = audience,
...
for (const algorithm of ['ES256', 'RS256']) {
  test(`${algorithm}: jose-signed token verifies in Native and Wasm`, async () => {
    ...
    assert.equal(wasm[`verify_${algorithm.toLowerCase()}_claims`](token, JSON.stringify(jwk), issuer, audience), true);
  ...
  test(`${algorithm}: replacing a verification key retires tokens from the old key`, async () => {
    ...
    const oldToken = await makeToken(algorithm, oldPair.privateKey, oldPair.jwk.kid, { sub: 'old-key' });
    const newToken = await makeToken(algorithm, newPair.privateKey, newPair.jwk.kid, { sub: 'new-key' });
    const wasmVerify = algorithm === 'ES256' ? wasm.verify_es256 : wasm.verify_rs256;
