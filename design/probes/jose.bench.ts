...
for (const algorithm of ['ES256', 'RS256']) {
  ...
  const nativeNs = Number(
    execFileSync(binary, [`bench-${algorithm.toLowerCase()}`, token, jwkJson, String(iterations)], {
      encoding: 'utf8',
    }).trim(),
  );
  ...
  console.log(
    `${algorithm} (${iterations} verifies): built-in ${nativeNs.toFixed(0)}/${wasmNs.toFixed(0)} ns/op Native/Wasm, custom ${customNativeNs.toFixed(0)}/${customWasmNs.toFixed(0)} ns/op`,
  );
}
