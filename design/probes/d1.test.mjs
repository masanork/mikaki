...
beforeEach(async () => {
  const tables = ['atomic_guard', 'revocation_event', 'token_issue', 'assertion_use',
    'authorization_code', 'client_session', 'sso_session', 'app_connection',
    'signing_key', 'client_key', 'client', 'credential', 'account_security'];
  ...
  await db.batch([
    ...
    db.prepare("INSERT INTO authorization_code VALUES('codehash','c','sid',1,'https://app.example/cb','challenge',?,NULL,NULL)").bind(expiry),
  ]);
});
...
test('successful exchange activates sid; session API first-primary query runs', async () => {
  ...
  const result = await db.withSession('first-primary').prepare('SELECT sub FROM valid_client_session').first();
  assert.equal(result.sub, 'sub');
});

test('parallel D1 batches exchange one code only once', async () => {
  const a = parameters('a'); const b = parameters('b');
  await batch('accept-assertion.sql', a); await batch('accept-assertion.sql', b);
  const results = await Promise.allSettled([batch('exchange-code.sql', a), batch('exchange-code.sql', b)]);
...
test('zero-row update rolls back while separately consumed assertion remains', async () => {
  const p = parameters(); await batch('accept-assertion.sql', p);
...
test('changes() guards stale epoch; revocation event and epoch are atomic', async () => {
  const p = parameters(); await batch('accept-assertion.sql', p); await batch('exchange-code.sql', p);
  await batch('revoke-all.sql', { account_id: 'a', expected_epoch: 0, operation_id: 'logout' });
  assert.equal(await scalar('SELECT count(*) FROM valid_client_session'), 0);
  await assert.rejects(batch('revoke-all.sql', { account_id: 'a', expected_epoch: 0, operation_id: 'stale' }), /CHECK/);
...
test('expiry, grant change and key stop cannot consume code', async () => {
  ...
  for (const [mutation, restore] of [
    ["UPDATE authorization_code SET expires_at=0", "UPDATE authorization_code SET expires_at=CAST(strftime('%s','now') AS INTEGER)+600"],
