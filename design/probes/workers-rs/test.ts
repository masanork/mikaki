...
try {
  ...
  const issuedCodes = await Promise.all(
    codeReports.map(async (response) => {
      assert.equal(response.status, 200);
      return response.json() as Promise<{ code: string; expires_at: number; digest: string }>;
    }),
  );
  ...
  assert.equal(
    firstCode.digest,
    createHash('sha256').update(Buffer.from(firstCode.code, 'base64url')).digest('base64url'),
  );
  assert.notEqual(firstCode.code, secondCode.code);

  const signedResponse = await worker.fetch('/sign');
  assert.equal(signedResponse.status, 200);
  const { token, jwk } = (await signedResponse.json()) as { token: string; jwk: string };
  ...
  const rsaPrivateJwk = {
    ...rsaKeys.privateKey.export({ format: 'jwk' }),
    kid: 'workerd-rs256-probe',
  };
  const rsaPublicJwk = {
    ...rsaKeys.publicKey.export({ format: 'jwk' }),
    kid: 'workerd-rs256-probe',
  };
  const rsaResponse = await worker.fetch('/sign-rs256', {
    ...
    body: JSON.stringify({
      private: JSON.stringify(rsaPrivateJwk),
      public: JSON.stringify(rsaPublicJwk),
    }),
  });
  console.log('workers-rs: RS256 worker response received');
  assert.equal(rsaResponse.status, 200);
  const { token: rsaToken, jwk: rsaJwk } = (await rsaResponse.json()) as {
    token: string;
    jwk: string;
  };
  ...
  console.log(
    'workers-rs: D1 atomicity, ES256/RS256 WebCrypto signing, Rust JWK/JWS validation, and OIDC code preparation passed',
  );
} finally {
  await worker.stop();
}

const adapterConfig = fileURLToPath(
  new URL('../../../crates/worker/wrangler.jsonc', import.meta.url),
);
const adapterScript = fileURLToPath(
  new URL('../../../crates/worker/build/worker/shim.mjs', import.meta.url),
);
