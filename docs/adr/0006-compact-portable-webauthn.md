# ADR 0006: コンパクトなWebAuthnコアを軸にsakimoriを発展させる

2026-09-22 / 採用。ユーザーによる開発継続・優先順位の決定を記録する。

## 背景と代替案

monbanまたはiwatoの発展形に統合する案と、sakimoriを継続する案を再検討した。既存実装の対応範囲・検証資産には価値がある。一方、sakimoriでは小さい検証境界、原子的な状態遷移、失効、ブラウザー内の秘密管理を中心に設計してきた。この土台を磨くことを優先する。

## 決定

- コンパクトで洗練された実装の提供を最優先とし、sakimoriの開発を継続する。Vaultなどの独自機能は、その土台の上で段階的に提供する。
