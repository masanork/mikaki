...
The workflow validates all three BOMs with a SHA-256-pinned CycloneDX CLI binary. The Rust SBOM is attached to the Worker archive as an SBOM attestation. The npm inventory describes build tooling and is distributed separately; it is not presented as a runtime dependency list.
...
After downloading an `attested-worker-<commit>` workflow artifact, verify its build provenance and Rust SBOM attestation:
