...
| Hosted run | The local Colima OIDF suite ran Config OP and a complete Basic OP plan against an ephemeral HTTPS workerd fixture. `oidcc-server` passed through passkey login. No public issuer or production D1 deployment exists. | Finish local plan review, then test a separate public conformance deployment. |
