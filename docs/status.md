...
| Vault | An owner-only R2 bucket, D1 migration, cleanup trigger, and page are deployed. Local Chromium exercises enrollment with PRF, encrypted name creation, reload/unlock, and update through the Worker. | Depends on an existing SSO session and PRF-capable passkey; no production write/unlock evidence yet. Existing credentials without PRF cannot open the Vault, and same-account replacement/recovery is missing. |
