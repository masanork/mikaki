...
| Normal-profile Worker | `https://mikaki.tossa.app` serves health, Discovery, JWKS, and deployed account, client, session, and Vault routes. One administrator account and credential exist in production. The administrator saved and reopened a Vault name in production. | No registered production RP has been demonstrated. See [deployment](cloudflare-deployment.md). |
...
| Vault | An owner-only R2 bucket, D1 migration, cleanup trigger, and page are deployed. Local Chromium exercises enrollment with PRF, encrypted name creation, reload/unlock, and update through the Worker. On 2026-09-24 the production administrator saved `name` with a real Passkey, reloaded the page, and decrypted the saved value; D1 reported active revision 1. | Cross-device restore, same-account replacement/recovery, and production conflict/failure cases remain unverified. Credentials without PRF cannot open the Vault. |
...
| Post-quantum cryptography | ML-KEM-768 Vault recipient sender and receiver candidate modules are in product source. Node, Chromium, and independent RustCrypto tests exercise their HPKE envelope and binding checks. Earlier isolated ML-KEM/ML-DSA probes cover native, Wasm, browser, and known-answer paths. | No post-quantum algorithm is enabled for product passkeys, OIDC signing, or live Vault sharing. The candidate is not wired to Vault writes or UserInfo release. See the [rollout plan](pqc-rollout.md). |
