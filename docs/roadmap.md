...
| Selective UserInfo claim sharing | Add a separate recipient-key directory, browser-side envelope, grant, and dedicated claim Worker. | Generation-1 recipient key is provisioned and active; browser envelopes, grants, and end-to-end sharing remain. See [claim sharing](vault-claim-sharing.md) and [key lifecycle](vault-recipient-key-lifecycle.md). |
