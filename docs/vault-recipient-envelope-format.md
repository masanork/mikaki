...
**Candidate, 2026-09-24. No product envelope may be written in this format yet.** The active generation-1 UserInfo key is a raw ML-KEM-768 encapsulation key, not an OIDC signing key or a JWK. The independent claim Worker holds its 64-byte decapsulation seed. The browser [sender](../crates/worker/ui/vault-recipient-envelope.ts) and claim Worker [receiver](../crates/userinfo-claim-worker/src/envelope.rs) now implement the candidate as product source modules. The [independent Node harness](../design/probes/pqc/product-envelope.test.ts) and [Chromium harness](../design/probes/pqc/browser.ts) open browser-generated envelopes with RustCrypto and check binding failures. The [claim Worker unit test](../crates/userinfo-claim-worker/src/envelope.rs) opens the checked-in noble fixture. These tests use a draft-04 domain label. They do not exercise a live Secrets Store binding, a Vault write, or claim release.
