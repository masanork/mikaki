...
Normalize all effective keys to ASCII dotted paths, durations to integer seconds, include `schema_version`, serialize a lexically key-sorted whitespace-free ASCII JSON object, and take lowercase hex SHA-256 as `policy_revision`. Comments or `60s` versus `1m` do not change it. New types require a schema revision. Compare the [Python design validator](../scripts/check_design.py) against Rust behavior. Keep revision in server records/audit; it is not a required public token claim.
