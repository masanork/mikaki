...
2026-09-22の公式リポジトリ調査では、jsonwebtokenのmaster manifestは11.1.0、rust_crypto featureはrsa/p256/ed25519-dalek等をまとめて有効化する構成だった。これは公開済み安定版の確認でも本番lockでもない。openidconnect-rsのmain manifestは4.0.1で、既定HTTP依存はreqwest/rustls。Workers向けに既定featureを無検討で有効化しない。
