...
G0のWebAuthn検証は従来の3 crateで進める。G1のOIDC実装に着手した時点でsakimori-oidcを追加する。worker → oidc → auth → webauthnとし、workerはauthの管理APIも直接呼べる。oidcはWorkers/D1/HTTPクライアントの型に依存しない。空crateを先行作成することは求めない。
...
組込みbackendを無効にした独自CryptoProviderでもES256/RS256検証を保ったまま308 KB（gzip 121 KB）に縮んだ。probeではissuer/audience/expiryとduplicate claimも確認した。Native/Wasmの10,000回測定ではES256が約216/681 µs、RS256が約228/417 µs。毎回JWKから鍵を作るため、cache済み鍵の性能値ではない。RSA JWKのn/eをDERへ変換する独自前処理が必要である。[crypto module](https://docs.rs/jsonwebtoken/11.1.0/jsonwebtoken/crypto/)の`JwtSigner`は同期traitのためasync KMS/WebCrypto signerを直接統合できないが、Node WebCryptoで非同期生成したES256署名を公開`Jws`構造体へ格納し、Native/Wasm検証するprobeは成功した。アプリ側にbase64url envelope生成を置けば非同期署名との境界を分離できる見込みがある。Cloudflare Workersの署名API・鍵形式・エラー処理は未検証。時刻注入、キャッシュ済み鍵の性能、配備先での署名、実D1との統合を確認して依存選択を確定する。依存spikeの再実行・制約は[隔離JOSE probe](../design/probes/README.md#rust-jose-依存スパイク初期評価)に記録した。
